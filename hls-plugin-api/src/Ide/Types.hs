...
pluginResponsible uri pluginDesc
    | Just fp <- mfp
    , T.pack (takeExtension fp) `elem` pluginFileType pluginDesc =
        True
    | otherwise = False
  where
...
class PluginMethod Request m => PluginRequestMethod (m :: Method FromClient Request) where
    ...
    combineResponses ::
        ...
        ResponseResult m
    default combineResponses ::
        Semigroup (ResponseResult m) =>
        SMethod m ->
        Config ->
        ClientCapabilities ->
        MessageParams m ->
      ...
      where
        ...
        wasRequested (InR ca)
            ...
            , -- See https://github.com/microsoft/language-server-protocol/issues/970
              ...
              Just caKind <- ca ^. kind =
                any (\k -> k `codeActionKindSubsumes` caKind) allowed
            | otherwise = False
...
instance PluginRequestMethod TextDocumentDocumentHighlight

instance PluginRequestMethod TextDocumentReferences

instance PluginRequestMethod WorkspaceSymbol

instance PluginRequestMethod TextDocumentCodeLens

instance PluginRequestMethod TextDocumentRename
...
instance PluginRequestMethod TextDocumentPrepareCallHierarchy

instance PluginRequestMethod TextDocumentSelectionRange where
    combineResponses _ _ _ _ (x :| _) = x

instance PluginRequestMethod CallHierarchyIncomingCalls

instance PluginRequestMethod CallHierarchyOutgoingCalls
...
-}
class PluginMethod Notification m => PluginNotificationMethod (m :: Method FromClient Notification)

instance PluginMethod Notification TextDocumentDidOpen

instance PluginMethod Notification TextDocumentDidChange

instance PluginMethod Notification TextDocumentDidSave

instance PluginMethod Notification TextDocumentDidClose
...
instance PluginNotificationMethod TextDocumentDidOpen

instance PluginNotificationMethod TextDocumentDidChange

instance PluginNotificationMethod TextDocumentDidSave

instance PluginNotificationMethod TextDocumentDidClose

instance PluginNotificationMethod WorkspaceDidChangeWatchedFiles

instance PluginNotificationMethod WorkspaceDidChangeWorkspaceFolders

instance PluginNotificationMethod WorkspaceDidChangeConfiguration

instance PluginNotificationMethod Initialized
...
data PluginCommand ideState = forall a.
      (FromJSON a) =>
    PluginCommand
    { commandId :: CommandId
    , commandDesc :: T.Text
...
mkFormattingHandlers f =
    mkPluginHandler STextDocumentFormatting (provider STextDocumentFormatting)
